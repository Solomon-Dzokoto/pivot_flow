    ...
    <div className={`flex items-center justify-center ${className}`}>
      <div className={`${sizeClasses[size]} relative`}>
        ...
        <div className="absolute inset-0 animate-spin">
          {/* Use text-accent for the SVG fill */}
          <svg viewBox="0 0 24 24" className="w-full h-full fill-current text-accent">
        ...
        {/* Orbital ring - Use border-accent for the border color */}
        <div className="absolute inset-0 border-2 border-t-transparent border-accent rounded-full animate-spin"
  ...
  return (
    <div className={`animate-pulse ${className}`}>
      {Array.from({ length: lines }).map((_, i) => (
        // Use bg-muted or bg-secondary for skeleton lines for theme adaptability
        <div key={i} className="h-4 bg-muted/70 dark:bg-muted/50 rounded-xl mb-2 last:mb-0"
